...
It is not only about spending less: whatever is left free on the last sheet is
an offcut you are going to use again. The lower the parts sit, the larger and
more unbroken that strip stays.
...
| **Aims for** | using the fewest sheets and, on the last one, leaving the largest possible strip free |
...
The result tells you three things: **how many sheets** were needed, **what
percentage** of that surface ended up covered by parts, and the **leftover** —the
millimetres of free strip on the last sheet, measured from where the tallest
part ends to the edge. That last number is the one that tells you what offcut
...
| Ángulos | `--angulos` | Candidate rotations, comma separated. |
...
| Resolución | `--resolucion` | Millimetres per pixel of the raster. Finer lays out slightly better and takes much longer. |
...
.venv/bin/pytest                     # the whole suite (856, ~4 min)
